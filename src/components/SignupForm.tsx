...
export default function SignupForm({ dark = false }: { dark?: boolean }) {
  ...
  return <div className="signup-form" data-dark={dark}>
    ...
    <form onSubmit={submit}>
      ...
      <div className="signup-controls"><input id={uid} className="dh-input" type="email" name="email" autoComplete="email" required maxLength={254} value={email} onChange={e=>setEmail(e.target.value)} disabled={status==='ok'||status==='loading'}/><button type="submit" className="dh-btn" disabled={status==='ok'||status==='loading'}>{status==='loading'?'Registering…':status==='ok'?'Request received':'Subscribe'}</button></div>
